	  ...
	  <?php
		    ...
		    if (IsSet($_POST ["campo"])){
				...
				if (!$conexao)
			  	...
			  	$valor = $_POST ["valor"];
			  	if ($campo == "desc"){
			  		$query =  "SELECT p.id, p.cod, p.descricao, p.unidade, p.estoque, p.cod_bar, e.nome, p.preco_comp, p.margem, p.ncm, p.tipo FROM tb_produto AS p INNER JOIN tb_empresa AS e ON p.descricao LIKE '%".$valor."%' AND p.id_emp = e.id AND (p.tipo ='VENDA' or p.tipo ='TINTA') ORDER BY cast(p.cod as unsigned integer);";
			  	}
			  	else
			  	if ($campo == "cod"){
			  	...
			  	if ($campo == "tinta"){
			  		...
			  		$query =  "SELECT p.id, p.cod, p.descricao, p.unidade, p.estoque, p.cod_bar, e.nome, p.preco_comp, p.margem, p.ncm, p.tipo FROM tb_produto AS p INNER JOIN tb_empresa AS e ON  p.tipo ='TINTA' AND p.id_emp = e.id ORDER BY e.nome;";
				...
				echo"  <div class=\"page_form\" id=\"no_margin\">
						<table class=\"search-table\" id=\"tabItens\">
					        ...
					        while($fetch = mysqli_fetch_row($result)){
					            ...
					            echo "<tr class='tbl_row' id='".$fetch[1]."'><td class=\"center_text\" >" .$fetch[1] . "</td>".
								     	 "<td class='nome'>" .$fetch[2] . "</td>".
								         ...
								         "<td class=\"center_text\">" .$fetch[4] . "</td>".
								     	 ...
								     	 "<td>" .money_format('%=*(#0.2n', $preco). "</td></tr>";
...
<div class="overlay">	
  <div class="popup">
    ...
    <div class="content">
      <div class="cadastro"><table><tr><td><button id="btnEdtProd">Editar</button></td><td><button class="'+cod+'" id="btnDelProd">Deletar</button></td></tr></table></div>
    </div>
  </div>
</div>
